...
function BookDataFactory($http) {
    ...
    function searchBook(title,page) {
        return $http.get("/api/searchBooks?title="+title+"&page="+page).then(complete).catch(failed);
    }
    
    function searchBookCnt(title) {
        return $http.get("/api/searchBooksCnt?title="+title).then(complete).catch(failed);
    }
