...
module.exports.searchGameCnt = function(req,res) {
    const gameTitle = req.query.title;

    var search = {};
    if (gameTitle) {
        let re = new RegExp(gameTitle, 'i')
        search = {"title": re}
    }

    Game.countDocuments(search, function(err,cnt){
        const response = {
            status: 200,
            message: cnt
        }
...
module.exports.searchGame = function(req,res) {
    ...
    Game.find(search, function(err,result) {
        const response = {
            status: 200,
            message: result
        }
